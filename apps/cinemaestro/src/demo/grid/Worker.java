...
public class Worker implements ManagementCallback, StatisticsCallback {
    ...
    public Worker(String type, boolean serIbis) throws Exception { 
        comm = new Comm(false, this, serIbis);
        
        if (type == null) { 
            throw new Exception("Worker requires type!");
        }
        
        comm.registerAtMaster(type);
    }
    
    private synchronized void setComponent(ComponentDescription c) { 
        
        if (workerComponent != null) { 
            System.out.println("[*] EEP: received more than one worker!");
            return;
        }
    ...
    private void executeRequest(Request r) { 
        try {
            comm.sendManagementMessage(r.execute(comm.getLocalID()));
        } catch (Exception e) {
    ...
    private void processMessage(Object data) { 
        ...
        } else if (data instanceof ComponentDescription) { 
            setComponent((ComponentDescription) data);
        } else if (data instanceof Boolean) { 
            setStart((Boolean) data);
        } else { 
            System.out.println("[*] EEP: got unknown message: " + data);
        }
    }
    ...
    private void createComponent(ComponentDescription c) throws Exception { 
        ...
        thread = create(clazz, c, input, output);
    }
    ...
    private void createPutQ(String name, QueueDescription putQ) throws IOException { 
        ...
        if (gets.length == 1) { 
            ...
            SimpleImageQueue q = SimpleImageQueue.create(putQ.getType(), 
                    putQ.getName(), putQ.getPutLength());
            ...
            RoundRobinOutputQueue q = RoundRobinOutputQueue.create(putQ.getType(),
                    putQ.getName(), putQ.getPutLength(), gets.length);
    ...
    private void createGetQ(String name, QueueDescription getQ) throws Exception { 
        ...
        if (puts.length == 1) { 
            ...
            SimpleImageQueue q = SimpleImageQueue.create(getQ.getType(), getQ.getName(), 
                    getQ.getGetLength());
            ...
            RoundRobinInputQueue q = RoundRobinInputQueue.create(getQ.getType(),
                    getQ.getName(), getQ.getGetLength(), puts.length);

            // FIXME: This may change the image order!
            //   MultiDoneImageQueue q = MultiDoneImageQueue.create(getQ.getType(), 
            //           getQ.getName(), getQ.getGetLength(), puts.length);
    ...
    public void start() throws Exception { 
        ...
        if (putQ != null) { 
            ...
            System.out.println("[*]   length " + putQ.getPutLength());
        ...
        System.out.println("[*] Started component " + workerComponent.getName());
        ...
        for (ImageQueue q : queues.values()) { 
            q.printStats();
        }
