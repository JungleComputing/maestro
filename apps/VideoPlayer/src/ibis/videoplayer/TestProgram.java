...
public class TestProgram {
    ...
    public static void main(String[] args) {
        try {
            UncompressedImage img = Image.load(new File(
                    "images/japan-tuin-8bit.ppm"), 0);
            System.out.println("Loaded " + img);
            // img = img.scaleUp( 3 );
            img = img.sharpen();

            // String inc = RenderFrameJob.readFile( new File(
            // "frames/context.inc" ) );
            // String scene = RenderFrameJob.readFile( new File(
            // "frames/S01-frame000000.pov" ) );
            // System.out.println( "Loaded scene" );
            // UncompressedImage img24 = RenderFrameJob.renderImage( 1500, 500,
            // 0, 1000, 0, 500, 0, inc + scene );
            // System.out.println( "Rendered scene" );

            // UncompressedImage img = UncompressedImage.load( new File(
            // "images/japan-tuin-16bit.ppm" ), 0 );
