...
public class BuildVideoProgram {
	...
	private void run( int frameCount, boolean goForMaestro ) throws Exception
	{
		...
		final int fragmentCount = (frameCount+Settings.FRAME_FRAGMENT_COUNT-1)/Settings.FRAME_FRAGMENT_COUNT;
		final Listener listener = new Listener( fragmentCount );
		final Job getFrameTask = BuildFragmentTask.createGetFrameJob( jobList );
		final Job playJob = jobList.createJob( "videoplayer", new BuildFragmentTask( getFrameTask ) );
		...
		if( node.isMaestro() ) {
			...
			for( int frame=0; frame<frameCount; frame += Settings.FRAME_FRAGMENT_COUNT ){
				...
				final FrameNumberRange range = new FrameNumberRange( frame, endFrame );
