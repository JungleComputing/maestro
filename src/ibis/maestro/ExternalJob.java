...
public class ExternalJob implements Job {
    ...
    /** The list of output files we want to grab after execution. */
	...
	try {
	    while( true ) {
		// FIXME: sandbox creation has a race condition, but
		// File.createTempDir does not exist. Since I don't know
		// how to implement this robustly, I don't try to be smart.
		sandbox = new File( "/tmp/sandbox-" + label++ );
		if( !sandbox.exists() ) {
		    // A potential race condition could take advantage
		    // of this gap.
		    if( !sandbox.mkdir() ){
			// FIXME: more robust error handling.
			System.err.println( "Cannot create sandbox directory '" + sandbox + '\'' );
			return;
		    }
		    break;
		}
