...
public class Master extends Thread implements PacketReceiveListener<WorkerMessage>, JobContext
{
    ...
    /**
     * Adds the given job to the work queue of this master.
     * This method may wait for the queue to drain to a reasonable
     * level, and may therefore way for the queue to empty a bit.
     * Therefore, jobs generated by other jobs are not put in the queue
     * by this method. Yes, that may still lead to a large queue, and
     * yes, this causes the master to favour jobs that are already
    ...
    public void submitExternalJob( Job j )
    {
        ...
        synchronized ( queue ) {
            int maxQueueSize = workers.getWorkerCount()*Settings.JOBS_PER_WORKER;
            while( queue.size()>maxQueueSize ) {
        	if( Settings.traceMasterProgress ) {
        	    System.out.println( "Master: queue now has " + queue.size() + " jobs, waiting for it to drain to " + maxQueueSize );
        	}
        	try {
		    queue.wait();
