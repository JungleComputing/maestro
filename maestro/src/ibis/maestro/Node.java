...
public abstract class Node extends Thread implements PacketReceiveListener
{
    ...
    private final UpDownCounter idleProcessors = new UpDownCounter( -Settings.EXTRA_WORK_THREADS ); // Yes, we start with a negative number of idle processors.
	...
	final String terminatorStartQuotumString = System.getProperty( "ibis.maestro.terminatorStartQuotum" );
	final String terminatorNodeQuotumString = System.getProperty( "ibis.maestro.terminatorNodeQuotum" );
	final String terminatorInitialSleepString = System.getProperty( "ibis.maestro.terminatorInitialSleepTime" );
	final String terminatorSleepString = System.getProperty( "ibis.maestro.terminatorSleepTime" );
	try {
	    if( terminatorInitialSleepString == null ) {
		return null;
	    }
	    final long initialSleep = Long.parseLong( terminatorInitialSleepString );
	    double startQuotum;
	    double nodeQuotum;
	    long sleep;
	    ...
	    final Terminator t = new Terminator( startQuotum, nodeQuotum, initialSleep, sleep );
	    t.start();
	...
	final NodeInfo nodeInfo = nodes.get( m.source );   // The get will create an entry if necessary.
	...
	final TaskInstance failedTask = nodes.registerTaskFailed( msg.source, msg.id );
     ...
     * @param userId The identifier associated with this job instance.
     ...
     */
    public abstract boolean submit( Object input, Object userId, boolean submitIfBusy, JobCompletionListener listener, Job...choices );
	...
	if( task instanceof AtomicTask ) {
	    ...
	    final MapReduceHandler handler = new MapReduceHandler( this, mrt, message, runMoment );
	...
	try {
		...
		if( message == null ) {
			...
			synchronized( this ) {
			    final long overhead = System.nanoTime()-overheadStart;
