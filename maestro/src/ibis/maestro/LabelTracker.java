...
/**
 * Hands out unique labels, and keeps track of which ones have and have not been
 * returned. Allows labels to be returned out of order and more than once. At
 * all times we can tell which labels, if any, are still outstanding, or that
 * all labels we have handed out have been returned.
 * 
...
public class LabelTracker {
    /*
     * In principle we could just use a set, but as an optimization we represent
     * the range of labels starting from 0 as just the upper value of this
     * range, and we grow this range whenever possible.
     * 
    ...
    /**
     * A label as handed out by the <code>LabelTracker</code>. It should be
    ...
    /**
     * Returns the given label to the tracker. The labels do not have to be
     * returned in the same order they were issued, and the same label may be
	...
	if (TRACE) {
	    Globals.log.reportProgress("returnLabel(): endOfRange="
		    + endOfRange + " labelValue=" + labelValue + " setsize: "
		    + set.size());
	}
    ...
    /**
     * Wait until all labels have been returned. The only way of getting this
     * thread back is by returning all labels that were handed out.
     * 
     * @throws InterruptedException
     *             Thrown if an interrupt was sent.
     */
	...
	while (true) {
		...
		if (TRACE) {
		    Globals.log
			    .reportProgress("Waiting for labels: endOfRange="
				    + endOfRange + " labelValue=" + labelValue);
		}
		wait();
	    }
	}
