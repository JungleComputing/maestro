...
class ObsoletePacketSendPort {
    ...
    /**
     * Return an empty slot in the cache. Assumes there is a lock on 'this'.
     */
    ...
    /**
     * Create a cache slot for the given connection. If necessary evict the old
     * entry.
     */
    @SuppressWarnings("synthetic-access")
    private CacheInfo getAndReserveCacheInfo(DestinationInfo newDestination,
        ...
        try {
            ...
            port.connect(newDestination.ibisIdentifier,
                    Globals.receivePortName, timeout, true);
            cacheInfo.port = port;
        } catch (IOException x) {
            synchronized (this) {
    ...
    synchronized void printStatistics(PrintStream s, String portname) {
        s.println(portname + ": sent " + Utils.formatByteCount(sentBytes)
                ...
                + localSentCount.get() + " local sends; " + evictions
                + " evictions");
        if (sentCount > 0) {
    ...
    boolean tryToSend(IbisIdentifier id, Message msg, int timeout) {
        ...
        try {
            ...
            node.setSuspect(id);
            Globals.log.reportError("Cannot send a " + msg.getClass()
                    + " message to master " + id);
